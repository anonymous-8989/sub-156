...
```bash
$ python3 downstream/train_phase_recognition_autolaparo.py 
  --lr 1e-3 \
  --opt adamW \
  --nepochs 100 \
  --bs 512 \
  --cpdir 'path/to/store/checkpoint' \
  --logdir 'path/to/store/log' \
  --lmdb 'path/to/downstream_task/lmdb'\
  --labels 'path/to/downstream_task/annotation' \
  --seed 30 \
  --pretrained-weights 'path/to/our/SurgFM.pth'


$ python3 downstream/test_phase_recognition_autolaparo.py 
  --lmdb 'path/to/downstream_task/lmdb' \
  --models 'path/to/your/cpdir' \
  --labels 'path/to/downstream_task/annotation' \
```
